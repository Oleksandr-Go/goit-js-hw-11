...
`).join(""),new m(".gallery a",{captionsData:"alt",captionDelay:250,captionPosition:"bottom"}).refresh()}function h(){l.innerHTML=""}function v(){c.classList.add("is-visible")}function x(){c.classList.remove("is-visible")}const L=document.querySelector(".form"),b=document.querySelector('input[name="search-text"]');L.addEventListener("submit",a=>{a.preventDefault(),h();const s=b.value.toLowerCase().trim();s.length&&(v(),f(s).then(r=>y(r.hits)).catch(r=>console.log("Ваш запит не знайдено,",r.message)).finally(()=>x()))});
